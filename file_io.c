...
typedef struct {
	int login_len;
	int password_len;
	uint8_t key_random_crypt[CRYPT_KEY_LEN];
...
crypt_status_t FileWriteRun(crypt_info_t* data)
{
	...
	uint8_t key_random_temp[CRYPT_KEY_LEN] = {0};
	...
	//Encryption check
	uint8_t login_temp[SIZE_BUFF_LOGIN] = {0};
	uint8_t password_temp[SIZE_BUFF_PASSWORD] = {0};
